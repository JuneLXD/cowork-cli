...
| Path | What it is |
|---|---|
| `src/` | The `room` binary. `commands.rs` is the command implementations, `room.rs` the log format, `state.rs` proposal state, `hooks.rs` the editor hooks, `feedback.rs` bug reports, `templates.rs` the prompts. |
...
| `.ai-common/` | This repository's own rooms, prompts, and protocol, created by `room init`. Cursors under `.cursors/` are gitignored. |
| `.claude/`, `.codex/` | Editor hook configuration written by `room init`. |
| `CLAUDE.md`, `AGENTS.md` | The rules blocks the agents read, written by `room init`. |
